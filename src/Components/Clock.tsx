...
const Clock: FunctionComponent<IProps> = (props: IProps) => {
  ...
  const sectionItems: ReadonlyArray<JSX.Element> = updateFaceElements(
...
function updateFaceElements(
  ...
  if (defaultSections) {
    ...
    defaultSections.map((sectionItem: ISection) => {
      ...
      const setupStopAngle =
        cumulativeAngle + sectionItem.setupTime * MinuteInDegrees;

      const sectionStartAngle = setupStopAngle; // + sectionItem.setupTime * MinuteInDegrees; // !added setup
      cumulativeAngle +=
        (sectionItem.duration + sectionItem.setupTime) * MinuteInDegrees; // transform minutes to degrees

      // if the section ends before starting angle dont draw it. Also if start is over an hour away
      if (
        cumulativeAngle <= currentPosition ||
        currentPosition < cumulativeAngle - CircleInDegrees
      ) {
