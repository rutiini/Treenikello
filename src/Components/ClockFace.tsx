...
const ClockFace: React.FC<IClockFaceProps & WithStyles> = (props: IClockFaceProps) => {
    const { classes } = props;

    // unaffected by props, memoize.
    const majors: ReadonlyArray<JSX.Element> = React.useMemo(() => {
        const mjrs: JSX.Element[] = [];
        for (let i = 0; i < 60; i++) {
            // the "big hours"
            if (i % 15 === 0) {
    ...
    return (
        <svg id="clock" className={classes.clock} viewBox="0 0 100 100">
            ...
            <circle className={classes.face} cx={props.centerCoordinate} cy={props.centerCoordinate} r={45} />
            {props.sectionItems}
            <circle className={classes.faceCover} cx={props.centerCoordinate} cy={props.centerCoordinate} r={41} />
            <g id="minuteMarkers">{majors}</g>
            <g id="hands">
                <rect
                    transform={`rotate(${props.clockData.getHourPosition()} ${props.centerCoordinate} ${
                ...
                />
                <rect
                    transform={`rotate(${props.clockData.getMinutePosition()} ${props.centerCoordinate} ${
                        props.centerCoordinate
                    })`}
                    id="min"
                    className={classes.hourMin}
                ...
                />
                <g
                    transform={`rotate(${props.clockData.getSecondPosition()} ${props.centerCoordinate} ${
                        props.centerCoordinate
                    })`}
                    id="secHand"
                >
            ...
            <circle className={classes.midPoint} cx={props.centerCoordinate} cy={props.centerCoordinate} r={3} />
        </svg>
    );
};
