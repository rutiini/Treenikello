...
test(`updating section does not do anything if there is no edit section`, () => {
    expect(updateSectionInActiveExercise(DefaultAppState, section)).toEqual(DefaultAppState);
});
...
test(`adding new section in active exercise successfully`, () => {
    ...
    expect(resultState.activeExercise.defaultSections.indexOf(section)).toBeTruthy();
    expect(resultState.exercises[0].defaultSections.indexOf(section)).toBeTruthy();
});
